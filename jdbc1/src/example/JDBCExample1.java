...
public class JDBCExample1 {
	...
	public static void main(String[] argsp) {
		 ...
		 * 
		 * 	- Connection을 구현한 객체를 통해서 SQL을 전달하고 결과를 반환 받을 수 있다!
		 * 
		 * 2) Statement
		 * 	- SQL을 DB에 전달
		 ...
		 * 	  원하는 DB와 연결할 수 있는 Connection 객체를 생성하는 역할의 객체
		 * 	=> 디비버의 연결할때 처음 뜨는 창 같은 애
		 */
		...
		try {
			...
			conn = DriverManager.getConnection(
					...
					,userName // 사용자 계정명
					,password); // 비밀번호
			// => 2-1 에서 로드된 JDBC 드라이버 + 연결 정보를 이용해 
...
//			System.out.println(conn); // conn 객체 생성 확인
			 ...
			 * 
			 * !! 주의 사항 !!
			 * - JDBC 코드에서 SQL 작성 시 마지막에 세미콜론(;) 작성 X
			 * 	 => 작성 시 : "SQL 명령어가 올바르게 종료되지 않았습니다" 예외 발생
			 * 	(왜? JDBC는 한 번에 SQL을 1개 씩만 실행 => DB에 SQL 전달 시 자동으로 끝에 ; 붙여줌)
			 * */
			...
			/* stmt.executeQuery(sql)
			 	- SELECT 문이 작성된 sql을 DB로 전달해 수행 후
			...
			*/
			...
			// ojdbc10.jar에서 제공하는 OracleDriver 클래스가 없을 경우 발생하는 예외
